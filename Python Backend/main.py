...
eventlet.monkey_patch()  # Required for Socket.IO async

# --- Flask Setup ---
app = Flask(__name__)
app.config['SECRET_KEY'] = 'secret!'
app.config['SQLALCHEMY_DATABASE_URI'] = 'postgresql://postgres:1149@localhost/postgres'
...
class Document(db.Model):
    ...
    title = db.Column(db.String, nullable=False, default="Untitled Document")
...
def get_document(id):
    ...
    if not doc:
        doc = Document(id=id, title=f"Document {id}", content={})
        db.session.add(doc)
        db.session.commit()
...
def join_document(data):
    ...
    if not doc:
        doc = Document(id=doc_id, title=f"Document {doc_id}", content={})
        db.session.add(doc)
        db.session.commit()

    # send current content to user
    socketio.emit("load-document", doc.content, room=request.sid)

    @socketio.on("send-changes")
    def send_changes(delta):
        socketio.emit("receive-changes",
                      {"delta": delta, "username": username},
                      room=doc_id,
                      include_self=False)
    ...
    def save_document(data):
        doc.content = data
        db.session.commit()
        print(f"💾 Document {doc_id} saved by {username}")
