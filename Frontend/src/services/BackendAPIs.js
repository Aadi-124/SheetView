...
export const userLogin = (userNameOrEmail,password)=> axios.post(`${BASE_URL}/login`,{userNameOrEmail:userNameOrEmail,password:password,Fake:"FAKE"});

export const userRegistration = (userData) => axios.post(`${BASE_URL}/register`,userData);
...
export const uploadSingleFile = (file, uploadedBy, description = "", folder = "root") => {
  ...
  return axios.post(`${BASE_URL}/files/uploadSingle`, formData,{name:"Aaditya"});
};
...
export const deleteFileById = (fileId) =>
  axios.delete(`${BASE_URL}/files/${fileId}`, {
    headers: {
      "x-user-id": JSON.parse(sessionStorage.getItem("app.auth"))?.userId || "",
    },
  });

export const getStorageStats = () => {
  return axios
    .get(`${BASE_URL}/files/storage/stats`, {
      headers: {
        "x-user-id": JSON.parse(sessionStorage.getItem("app.auth"))?.userId || "",
      },
    })
...
export const listUserFolders = () => {
  return axios
    .get(`${BASE_URL}/files/folders`, {
      headers: {
        "x-user-id": JSON.parse(sessionStorage.getItem("app.auth"))?.userId || "",
      },
    })
