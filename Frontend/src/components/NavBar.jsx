...
import { clearLocalAuth } from "../services/localStorageAuth";
import { getLocalAuth } from "../services/localStorageAuth";
/* ------------------------------------------------------------------ */
...
export default function NavBar() {
  ...
  const handleLogout = () => {
    console.log("NAVBAR Before = "+JSON.stringify(getLocalAuth()));
    const ok = clearStoredAuth();
    clearLocalAuth();
    console.log("NAVBAR After = "+getLocalAuth());
    console.log();
    if (ok) {
      setAuth(null);
      setOpen(false);
